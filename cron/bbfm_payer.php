...
//!!!! Mind to fill your server IP line 1088 in $data[ 'allowed_notif_IPs' ]
...
$XFILE_PATH = "/var/www/bbfm_xfiles/";

$admin_email='email@admin.com';// to be replaced with real email address !
...
$JSON = jsonrpc_call( "curl --data '{\"jsonrpc\":\"2.0\", \"id\":1, \"method\":\"getinfo\", \"params\":{} }' http://127.0.0.1:6332" );

$Object = json_decode( $JSON );

echo print_r( $Object, true );

$count_unhandled = $Object->result->count_unhandled;
...
Please contact us via our slack support channel for further information :

https://byteball.slack.com/messages/C6M5SHMAA/

Best Regards.

The Byteball For Merchants Team
@byteball-for-merchants.com

";

                        $MailSujet = "Byteball duplicate payment notification";
...
//     }
    ...
    if( $row[ 'received_amount' ] < fee_bbfm( $row[ 'received_amount' ] )  ){
        ...
        $query = "update bbfm set global_status = 'error'";
        $query .= ", error_msg  = 'no byteball sent : received amount does not cover the outgoing fees'";

        $query .= " where id = '" . $row[ 'id' ] . "'";

        mysqli_query($mysqli, $query);

        cron_return_error( "\nno byteball sent : received amount does not cover the outgoing fees for payment id " . $row[ 'id' ], true );
    ...
    $shell_string = "curl --data '{\"jsonrpc\":\"2.0\", \"id\":1, \"method\":\"sendtoaddress\", \"params\":[\"" . $row[ 'address_merchant' ] . "\", " . $sent_amount . "] }' http://127.0.0.1:6332";

    echo "\nshell_string : " . $shell_string;

    $sendtoaddress_JSON = jsonrpc_call( $shell_string );

    $sendtoaddress = json_decode( $sendtoaddress_JSON );
    ...
    $query .= ", sent_unit  = '" . $sendtoaddress->result . "'";
...
//         error_log("\n" . 'query: ' . $query);
    ...
    if( $row[ 'global_status' ] == 'completed' or $row[ 'global_status' ] == 'error' ){
...
function email_notif_url_notification_error( $row, $url_notify ){
    ...
    $MailSujet ='*** Error on your byteball payment url notification for your sale ' . $row[ 'merchant_order_UID' ] . ' ***';

    $MailBody = "An error was encountered while trying to notify your byteball payment to your 'merchant_return_url'.
...
function email_notify( $row ){
    ...
    if( $row[ 'global_status' ] == 'error' ){
        ...
        $MailSujet ='*** Error on your byteball payment for your sale ' . $row[ 'merchant_order_UID' ] . ' ***';

        $MailBody = "An error was encountered while processing your byteball payment.
...
";
     ...
     */
        ...
        $MailSujet ='[confirmed] Byteball payment sent for your sale ' . $row[ 'merchant_order_UID' ];
...
The payment for the above mentioned order is now confirmed by the network.";
     ...
     */
        ...
        $MailSujet ='[unconfirmed] Byteball payment sent for your sale ' . $row[ 'merchant_order_UID' ];

        $MailBody = "Hello,

You just received from us a byteball payment for the above mentioned order! Confirmation is still required from the network.";
...
//     }
    ...
    if( $is_woocommerce or $is_bballs){
...
function listtransactions(){
    ...
    $JSON = jsonrpc_call( "curl --data '{\"jsonrpc\":\"2.0\", \"id\":1, \"method\":\"listtransactions\", \"params\":[] }' http://127.0.0.1:6332" );
     ...
     */
     ...
     if( $mode == 'get' ){
        ...
        $setopt_array = array(
            ...
            CURLOPT_MAXREDIRS => 3,
            CURLOPT_URL => $CURLOPT_URL . '?' . http_build_query( $data ),
            CURLOPT_USERAGENT => 'Byteball for Merchants',
        );
